...
class FishClawTools:
    ...
    def _ensure_browser(self) -> None:
        ...
        if HAS_STEALTH:
            ...
            _log("FishClaw: Stealth 模式已启用")

    def _get_page(self) -> Page:
        self._ensure_browser()
        # 检查浏览器是否仍然连接
        if self._browser is None or not self._browser.is_connected():
            _log("FishClaw: 检测到浏览器已断开，重新初始化...")
            self._close_browser()
            self._ensure_browser()
    ...
    def _save_cookies(self) -> None:
        ...
        _log(f"FishClaw: Cookies 已保存到 {self.cookies_path}")
    ...
    def _load_cookies(self) -> bool:
        ...
        try:
            ...
            _log(f"FishClaw: 已从 {self.cookies_path} 加载 Cookies")
            return True
        except Exception as e:
            _log(f"FishClaw: 加载 Cookies 失败 - {e}", "warning")
            return False
    ...
    def _is_logged_in(self, page: Optional[Page] = None) -> bool:
        ...
        # Check for session expired first (critical!)
        try:
            body_text = p.evaluate('() => document.body.innerText')
            if 'Session过期' in body_text or 'session过期' in body_text.lower():
                _log('FishClaw: session expired detected')
                return False
        except Exception:
            pass
        ...
        logged_in_indicators = [
            'a[href*="/personal"]',  # 个人中心链接
            'div[class*="user"]',     # 用户信息区域
            'img[class*="avatar"]',   # 头像
        ]
    ...
    def _ensure_logged_in(self) -> Optional[str]:
        ...
        try:
            ...
            if current_url and ("goofish.com" in current_url or "xianyu" in current_url):
                _log(f"FishClaw: 检测当前页面登录状态（{current_url[:60]}...）")
                if self._is_logged_in(page):
                    _log("FishClaw: 当前页面已登录，无需重新加载")
            ...
            if self._is_logged_in(page):
                _log("FishClaw: 登录状态有效")
                return None  # 已登录，放行

            # 未登录：拉起扫码流程
            _log("FishClaw: 未登录，自动触发二维码登录流程...")
            ...
            if "browser has been closed" in error_msg.lower() or "target closed" in error_msg.lower():
                _log(f"FishClaw: 浏览器已被关闭，正在重新初始化... ({e})", "warning")
                # 清理旧连接并重新初始化
                self._close_browser()
                # 递归调用自己重新检查登录
                return self._ensure_logged_in()
            else:
                _log(f"FishClaw: 登录检查时出错 - {e}", "error")
                raise
        ...
        for elapsed in range(timeout_seconds):
            ...
            try:
                if self._is_logged_in(page):
                    ...
                    self._save_cookies()
                    title = page.title()
                    _log(f"FishClaw: 扫码登录成功，用时 {elapsed + 1} 秒")
                    return f"登录成功！当前页面：{title}。Cookies 已保存到 {self.cookies_path}。"
            except Exception as e:
                _log(f"FishClaw: 登录检测异常 - {e}", "debug")
                pass
    ...
    def _navigate_to_publish(self, page: Page) -> Tuple[bool, str, Optional[Any]]:
        # First dismiss any login modal that might be blocking clicks
        self._dismiss_login_modal(page)

        # Method 1: Direct URL navigation (simplest, avoids modal blocking clicks)
        try:
            _log("FishClaw: navigating directly to publish page...")
            page.goto("https://www.goofish.com/publish", wait_until="domcontentloaded", timeout=15000)
            _random_delay(2.0, 3.0)
            self._dismiss_login_modal(page)
            # Wait a bit more for SPA to render after modal removal
            _random_delay(1.0, 2.0)
            if "publish" in page.url.lower():
                return True, f"已进入发布页面：{page.url[:80]}", page
        except Exception as e:
            _log(f"FishClaw: direct navigation failed: {e}", "warning")

        # Method 2: Click "发闲置" link on homepage
        try:
            _log("FishClaw: looking for publish link on homepage...")
            page.goto("https://www.goofish.com", wait_until="domcontentloaded", timeout=15000)
            _random_delay(2.0, 3.0)
            self._dismiss_login_modal(page)
            # Find the exact "发闲置" link (not the changelog one)
            publish_link = page.locator('a[href$="/publish"]').first
            if publish_link.is_visible(timeout=3000):
                _log("FishClaw: found publish link, clicking...")
                with self._context.expect_page(timeout=10000) as new_page_info:
                    publish_link.click(force=True)  # force click bypasses modal
                publish_page = new_page_info.value
                publish_page.wait_for_load_state("domcontentloaded", timeout=15000)
                _random_delay(2.0, 3.0)
                self._dismiss_login_modal(publish_page)
                return True, f"已进入发布页面：{publish_page.url[:80]}", publish_page
        except Exception as e:
            _log(f"FishClaw: clicking publish link failed: {e}", "warning")

        return False, "未能进入发布页面", None

    def _dismiss_login_modal(self, page: Page) -> None:
        """Close login modal if present (blocks all clicks on the page)."""
        try:
            # Check if modal is visible
            modal = page.locator('.ant-modal-wrap, [class*="login-modal"]').first
            if modal.is_visible(timeout=2000):
                _log("FishClaw: login modal detected, removing from DOM...")
                # Force remove via JS - more reliable than clicking
                page.evaluate("""() => {
                    const modals = document.querySelectorAll('.ant-modal-wrap, [class*="login-modal"]');
                    modals.forEach(m => m.remove());
                }""")
                _random_delay(0.5, 1.0)
                _log("FishClaw: login modal removed")
        except Exception:
            pass  # No modal found, that's fine
    ...
    def login(self, timeout_seconds: int = 180) -> str:
        ...
        try:
            ...
            error_msg = str(e)
            if "browser has been closed" in error_msg.lower() or "target closed" in error_msg.lower():
                _log("FishClaw: 浏览器已关闭，重新初始化后再次尝试登录", "warning")
                self._close_browser()
    ...
    def search_market(self, keyword: str, max_results: int = 20) -> str:
        ...
        try:
            page = self._get_page()
            _log(f"FishClaw [search_market]: 搜索关键词「{keyword}」")
...
"""
            ...
            for _ in range(30):
                try:
                    ...
                    _log(f"FishClaw [get_selling_items] JS 异常: {e}","warning")
    ...
    def manage_item(self, item_url: str, action: str) -> str:
        ...
        try:
            ...
            # 跳转到商品详情页
            _log(f"FishClaw [manage_item]: 前往 {item_url[:80]}...")
            ...
            for sel in btn_selectors:
                try:
                    ...
                    if el.is_visible(timeout=3000):
                        ...
                        el.click()
                        _random_delay(1.0, 2.0)
                        _log(f"FishClaw [manage_item]: 已点击「{action_text}」按钮（{sel}）")
                        clicked = True
                        break
                except Exception:
                    continue
            ...
            for sel in [
                ...
                try:
                    ...
                    if el.is_visible(timeout=3000):
                        ...
                        el.click()
                        _random_delay(1.0, 2.0)
                        _log(f"FishClaw [manage_item]: 已点击确认弹窗（{sel}）")
                        break
                except Exception:
                    continue
    ...
    def simulate_farming(self, duration_minutes: int = 5) -> str:
        ...
        try:
            ...
            _log(f"FishClaw [simulate_farming]: 开始养号，预计 {duration_minutes} 分钟...")
